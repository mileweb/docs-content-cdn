...
管理员可通过用户和权限管理页面上的**用户**页面来挂起用户。挂起用户的操作将暂时阻止该用户访问门户；但是该操作并不会删除用户的用户名、角色、密码和其他设置。系统不会通知用户他们已被暂停，这些用户只是无法再进行登录。

如果用户在被挂起时恰好已登录 Portal，那么该用户可以继续使用门户，但在其退出后无法将再次登录。已被挂起的用户在尝试登录时，CDN Pro 会显示一条消息，指出该帐户已挂起并引导用户寻找管理员确认。
