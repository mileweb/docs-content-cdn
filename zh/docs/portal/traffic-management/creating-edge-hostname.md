...
5. To use a DNS record to advertise your HTTPS support and allow clients to negotiate the supported protocols faster, click **Advanced Settings**, and fill in the following fields:
...
**Note**: CDN Pro also supports the following special server groups for customers with China access enabled: **nearChina**, **ChinaStandard**, and **ChinaPremium**. For more information, contact the [{{title}} support team](mailto:support@{{siteDomain}}).