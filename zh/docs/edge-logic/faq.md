...
<p align=center><img src="/docs/edge-logic/redirect-apex-domain.png" alt="重定向二级域名" width="700"></p>
...
CDN Pro采用令牌桶算法实现频率限制。令牌桶算法基于一个固定容量的桶，令牌以固定的速率添加到桶中。在允许一个API请求继续处理之前，CDN Pro 先查看桶中的令牌数量是否足以用于处理该请求。如果是，则从桶中扣除一定数量的令牌，然后放行API请求。如果可用的令牌数量不足，则会阻止请求，并返回错误信息，提示用户其接口调用在1分钟的滑动窗口内已超过频率限制。

令牌桶的容量和填充速率由客户管理API中`configs.apiMaxBurst`和`configs.apiRate`字段控制。例如，如果客户有`configs.apiMaxBurst = 45`和`configs.apiRate = 120`，则 CDN Pro 以每分钟120个令牌的速率向桶中添加令牌，桶的总容量为45。CDN Pro 以一种“贪心”的方式向令牌桶填充令牌。以`configs.apiRate = 120`为例，CDN Pro 每500毫秒就会向桶中增加1个令牌。换句话说，CDN Pro 并不会等待整整一分钟后一次性加入120个令牌。

* **接口权重**

并非所有 API 调用在服务器资源消耗方面都是相同的。为此，每个API接口都被分配了一个权重，用于确定每次接口调用时，需从令牌桶中扣除的令牌数量。这意味着您在频率限制内可以进行的有效调用次数取决于您所调用的接口组合。

大多数API接口都被分配了默认的令牌权重：1，因此每次接口调用消耗1个令牌。但是，一些接口（特别是报表接口）由于其显著更高的服务器资源使用率，被分配了更高的权重，每次接口调用会消耗更多令牌。

* **错误提示和处理**

在令牌被消耗之后，一个`x-rate-limit-remaining: X`响应头会被添加到API调用的HTTP响应中，表示桶中剩余的令牌数量。此响应头表示客户此时可以发出的API请求的剩余配额。失败或格式错误的请求也会从桶中消耗一定数量的令牌。如果桶中没有足够数量的令牌，API网关将返回一个HTTP 429错误，响应头为`x-rate-limit-retry-after-seconds: Y`，告诉客户端在Y秒后重试。
...
频率限制应用于客户级别。同一个客户下的所有API账号共享同一个令牌桶。过度使用一个账号将耗尽客户级别的配额。代理商的子客户的频率限制是相互独立的。此外，子客户的API调用不会使用代理商客户的配额。
