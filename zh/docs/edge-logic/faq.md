...
为了防止CDN Pro的API服务器过载，CDN Pro API强制限制客户每分钟可以发送的请求数量。如果客户发送过多的请求，API频率限制通过返回带有HTTP状态码429的错误消息来限制客户。

* **令牌桶算法**

CDN Pro采用令牌桶算法实现频率限制。令牌桶算法基于一个固定容量的桶的类比，令牌以固定的速度添加到桶中。在允许一个API请求继续之前，要检查桶，看它是否至少包含一个令牌。如果是，则从桶中删除一个令牌，允许API请求继续进行。如果可用的令牌数量不足，则会阻止请求，并出现一个错误，提示在1分钟的滑动窗口内已超过配额。
桶的容量和填充速率由客户管理API中`configs.apiMaxBurst`和`configs.apiRate`字段控制。例如，如果客户有`configs.apiMaxBurst = 45`和`configs.apiRate = 120`，则以每分钟120个令牌的固定速度向桶中添加令牌，桶的总容量为45。水桶以一种“贪婪”的方式被填满。CDN Pro试图尽快向桶中添加令牌。例如，每分钟120个令牌即每500毫秒增加1个令牌。换句话说，添加不会等待整整一分钟来再生整堆120个令牌。

* **指示灯和错误提示**

在令牌被消费之后，一个`X-rate-limit-remaining: X`响应头被添加到API调用的HTTP响应中，表示桶中剩余的令牌数量。此响应头表示客户此时可以发出的API请求的剩余配额。失败或格式错误的请求也会从桶中消耗一个令牌。如果桶中没有足够数量的令牌，API网关返回一个HTTP 429错误，响应头为`x-rate-limit-retry-after-seconds: Y`，告诉客户端在Y秒后重试。

* **避免频率限制错误的最佳实践**

检查API请求历史记录。`GET /ngadmin/apicalls` 显示了你调用的API。(调用此API需要客户管理员账号。)仔细检查记录，看是否有滥用的可能。
...
另一个例子是在一个请求中刷新多个文件，而不是为每个刷新URL调用API。如果有数千个刷新URL遵循某种模式，则在创建刷新请求时通过指定dirUrls或regexppatterns字段来使用目录或正则表达式刷新。请求的数量不应该随着您获得更多的域、属性和其他资源而增加。

如果通过脚本调用API，它应该对间歇性或非特定错误能够处理和应对。脚本应该执行`x-rate-limit-retry-after-seconds`报头，并在延迟后重试请求。考虑在您的代码中包含一个进程，它可以调节请求的频率，以便它们能均匀地随时间分布。

如果问题仍然存在，请联系我们的技术支持团队。如果有合理的需要增加频率限制或突发上限，技术支持团队将评估您的需求并提高阈值。


* **注意**

频率限制适用于客户级别。同一个客户下的所有API帐户共享同一个令牌桶。过度使用一个帐户将耗尽客户的配额。转售商的子客户的频率限制是相互独立的。此外，子客户的API调用不会使用父客户的配额。

因为CDN Pro UI也会调用API，所以Pro UI中的操作也使用API频率限制令牌。
