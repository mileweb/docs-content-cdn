...
检查API请求历史记录。`GET /ngadmin/apicalls` 显示了您调用的API(调用此API需要客户管理员账号)。请仔细检查记录，查看是否有滥用API的情形。

您也可以通过使用更适合该场景的API或将类似的请求合并为一个来减少请求的数量。例如，如果您想监控一个域列表的流量，一种较差的方法是:
```
...
```
另一个例子是，在一个请求中刷新多个文件，而不是为每个刷新URL单独调用API。如果有数千个刷新URL符合某种规律模式，则在创建刷新请求时通过指定`dirUrls`或`regexPatterns`字段来使用目录或正则表达式刷新。请求的数量不应该随着您的域名、加速项和其他资源的增加而增加。
...
频率限制适用于客户级别。同一个客户下的所有API帐户共享同一个令牌桶。过度使用一个帐户将耗尽客户的配额。零售商的子客户的频率限制是相互独立的。此外，子客户的API调用不会使用父客户的配额。
