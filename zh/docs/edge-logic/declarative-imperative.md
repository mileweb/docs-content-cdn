...
上面的规则 1 可能是 nginx 行为里新用户最难理解的，因为它和绝大部分编程语言都不一样。因此我们强烈建议用户避免在 `if` 块里使用声明式指令，可能的话，请使用[此网页](https://docdev.quantil.com/en/cdn/docs/edge-logic/multiple-origins)描述的替代方案。
