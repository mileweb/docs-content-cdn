...
## 2026年9月1日
#### 控制台变更
* 修复了权限名称在用户的语言偏好非英文时仍以英文显示的问题。
* 改进了调度域名的编辑功能，在未做更改即退出编辑模式时不再弹出不必要的确认提示。
* 改进了AI助手聊天面板，使其在打开时不再遮挡表单控件。
* 在联系人列表表格中新增了最后登录时间的显示与排序功能。
* 修复了“刷新类型”下拉框选项文本被截断的显示问题。
* 支持在管理后台查看 CDN Pro 服务配额。
* 完善了管理后台基于角色的访问控制。

## 2026年8月19日
#### API 变更
* 修复了为客户、联系人或 API 账号添加或更新产品时返回内部错误的问题。
* 新增了对部署任务及其他版本相关 API 中版本号的校验，当版本号不是正整数时返回 400 响应及 `InvalidVersion` 错误码。
* 统一了证书版本不存在时返回的错误码，客户端会一致地收到 `InvalidCertVersion`。

#### 控制台变更
* 修复了在刷新或预取 URL 输入框中输入只包含空格的空行时，会错误地提示"该 URL 已添加"的问题。
* 修复了"调度域名请求"报表的调度域名列表中出现重复条目的问题。
* 修复了证书备注被截断的问题。
* 升级底层 React 与 Ant Design 框架。
* 增强了边缘逻辑 AI 助手，使得附加到消息上的边缘逻辑会记录在会话历史中，并在您重新打开历史会话时显示。

## 2026年8月4日
#### API 变更
* 修复了创建日志配置时响应中 `Location` 头部不正确的问题。
* 改进了子客户的 API 请求统计。
* 支持记录客户的历史产品订阅信息。

#### 控制台变更
* 边缘逻辑 AI 助手新增Agent模式，可直接修改边缘逻辑配置。修改内容会以 diff 形式展示，您可以逐条审阅后选择接受或拒绝。
* 改进了无权限访问时的错误处理，各页面的提示信息更加清晰、一致。
* 改进了自助注册和账号邀请流程，加强了用户名的校验。
* 改进了证书、刷新、验证任务和部署任务页面的筛选功能，"最近 1/2/6 小时"快捷筛选在不同时区下的结果更加准确。
* 优化了保密信息详情页面的按钮样式。
* 优化改进了账号/权限管理后台。

## 2026年7月22日
#### API 变更
* 上线了新的API接口，支持查询API请求的统计数据。
* 改进了服务端处理加速项目和证书大批量部署时的性能。

#### 控制台变更
* 上线了账号/权限管理后台（阶段3）。当前已支持对组织信息、API账号及控制台账号等完整的管理功能。
* 改进了角色与权限的逻辑和实现。
* 限制非活跃客户使用控制台功能。
...
#### 控制台变更
* 上线了账号/权限管理后台（阶段2）。
* 改进了对已挂起客户账号的处理，根据挂起原因显示更明确的消息并提供适当的自助操作。
* 支持查看指定子客户的报表数据。
* 修复了已登录用户在 CDN Pro 与 HDT 控制台之间切换时被登出的问题。

## 2026年6月23日
#### API 变更
* 修复了中文邀请邮件中包含英文法律文件链接的问题。
* 支持记录客户账号被挂起的原因。
* 优化管理员API账号的权限控制。

#### 控制台变更
* 修复了禁用 AWS S3 认证后源站配置未保存的问题。
* 优化了各页面数据查询异常时的错误处理。
* 上线了账号/权限管理后台（阶段1）。

## 2026年6月10日
#### API 变更
* 修复了 API 请求列表查询响应中 `count` 字段返回当前页大小而非总记录数的问题。
* 改进了证书版本的验证逻辑。

#### 控制台变更
* AWS S3 源站认证中的**密钥**字段现在要求从下拉列表中选择一个密钥，值会自动格式化为 `$SECRET(name)`，不再支持直接输入明文密钥。
* 修复了克隆加速项目时**附加 TLS 证书**字段被禁用的问题。
* 修复了用户语言偏好非英语时，失败的测试用例结果显示为绿色而非红色的问题。

## 2026年5月27日
#### API 变更
* 在资源查询响应中新增 `Last-Modified` 响应头，并支持更新操作中的可选请求头 `If-Unmodified-Since`，允许调用方避免覆盖并发修改。
* 加速项目配置中的 `origins.authentication.awsS3.secretKey` 字段现在要求使用 `$SECRET(name)` 格式，不再支持明文密钥。
* 修复了当引用的密钥被重命名或删除后，加速项目验证返回内部错误的问题。
* 改进了日志下载格式无效变量的错误提示，改为按字母顺序列出支持的变量。

#### 控制台变更
* 改进了控制台账号邀请流程的国际化支持，邀请邮件和页面现在使用受邀者的偏好语言。
* 修复了自助注册时选择的语言偏好未应用于邀请邮件和确认页面的问题。

## 2026年5月13日
#### API 变更
* 修复了当请求频率限制令牌桶耗尽时，429 响应中缺少频率限制相关响应头的问题。
* 修复了代理操作时，父账号可以代替非活跃客户执行写操作的问题。

#### 控制台变更
* 在统计报表中新增了表格视图，更便于对各分组数据进行排序和比较。
* 修复了从部署任务或验证任务详情页返回后，列表跳回第一页而非恢复原来位置的问题。
* 修复了新建刷新请求后未能立即显示在刷新列表中的问题。
* 修复了从待命任务页面返回时，部署确认弹窗再次弹出的问题。
* 修复了统计报表中提示信息未本地化的问题。

## 2026年4月29日
#### API 变更
* 完善部署通知，修复特定场景下证书部署未发送通知的问题。
* 改进报表接口，完善服务类型的分组逻辑。

#### 控制台变更
* 发布 AI 智能助手，用于帮助用户配置边缘逻辑。
* 调整控制台登录页账号信息校验的逻辑。
* 修复了无法保存用户时区选择的问题。
...
#### API 变更
* 修复了证书列表查询权限控制上的问题。
* 修复了当API账号变更时，特定情况下未能触发邮件通知的问题。
* 修复了API请求频率限制相关的响应头。

#### 控制台变更
* 在加速项目的**高级设置**中新增**缓存键URI**字段，允许用户设置缓存键中使用改写前还是改写后的URI。
* 将**HTTP与HTTPS分开缓存**字段从**TLS设置**迁移至**高级设置**，与其他缓存键相关设置归组在一起。
* 调整加速项目配置页签顺序，**高级设置**现在显示在**实时日志**之前。
* 更新证书过期时的部署告警提示信息，使提示信息提醒过期的证书的类型。
* 修复了右上角用户信息面板未自动关闭的问题。
* 账单页面支持查看最近几个月的费用。此功能只面向自注册客户。
* 账单页面支持查看历史账单以及管理付款方式。此功能只面向已转正的自注册客户。

## 2026年4月1日
#### API 变更
* 处理部署任务时，改进边缘证书，回源鉴权证书和CA证书的校验逻辑，区分错误码。
* 修复历史加速域名查询接口分页处理相关的逻辑。
* 修复调度域名接口中`weight`参数相关的错误信息。
* 改进接口频率限制的实现，提高性能。
* 改进批量部署证书和加速项目的性能。
* 降低加速项目查询接口的令牌权重。

#### 控制台变更
* 日志格式配置编辑器中的变量说明，支持国际化。
* 改进加速项目页面下实时日志相关字段的校验逻辑。
* 改进日志格式Id在控制台上的展示方式。
* 改进证书列表页，证书关联加速项目的Id的展示方式。
* 改进加速项目列表页面的高级查询功能。

## 2026年03月17日

#### API变更
* 改进了部署任务中对于已过期证书的校验方式，便于更好地区分边缘证书、回源鉴权证书和CA证书。
* 自动处理URL中资源ID尾部的空格，避免请求异常。
* 允许API账户的IP白名单覆盖根账号的IP白名单，从而实现针对每个API账户更精细的访问控制。
* 修复了API账号IP白名单评估的逻辑，避免父客户操作子客户时访问控制异常。

#### 控制台变更
* 为自助注册客户添加了账单页面，提供月度费用明细，并支持通过Stripe页面管理历史发票和支付方式。
* 修复了加速项目下TLS证书字段的显示问题。
* 改进了自助注册页面。
* 改进了控制台中多处文本的本地化。
...
#### API变更
* 在客户列表 API 响应中新增了`referrer`字段。
* 将每个证书的最大版本数从 500 提升至 1000。
* 改进了各类通知，确保在所有支持的语言中使用对应语言的链接。
* 改进了 API 限流机制，支持为每个接口配置不同的令牌权重，从而对请求计数方式实现更精细的控制。
* 修复了将缩写形式的 IPv6 地址 :: 用作源服务器时，保存加速项目会返回内部错误的问题。

#### 控制台变更
* 在边缘逻辑编辑器和实时日志格式中新增了与 SSL 及服务器环境相关的变量：`$server_env`、`$ssl_cert_alg`、`$ssl_fingerprint_ja4`、`$ssl_fingerprint_ja3` 和 `$ssl_ciphers`。
* 提升了流量和带宽报表的数据精度，流量显示精度提升至 0.01 GB，带宽显示精度提升至至少 0.1 kbps。
* 在控制台登录页面新增了自助注册链接，方便尚未拥有账号的用户快速跳转至 CDN Pro 注册页面。
* 为登录页面和忘记密码页面新增了国际化（i18n）支持，所支持的语言与控制台现有语言保持一致。
* 修复了无法向支持 IPv6 的服务器发起测试请求的问题。
* 修复了测试用例集和测试用例页面上的多处易用性问题。
* 修复了无效 IPv6 地址也能通过源服务器验证的问题。
* 改进了源服务器设置，防止在"服务器"和"备服务器"字段中添加重复条目（相同的主机名/IP与端口的组合）。
...
* 新增了更多关于验证客户端证书的变量: (ssl_client_escaped_cert, ssl_client_fingerprint, ssl_client_i_dn, ssl_client_raw_cert, ssl_client_s_dn, ssl_client_serial, ssl_client_v_end, ssl_client_v_remain, ssl_client_v_start, ssl_client_verify).
