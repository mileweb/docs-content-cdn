...
| Edge Logic                | Write NGINX configuration code to specify how you want CDN360 to deliver content to your visitors. For more information, refer to CDNetworks' [Edge Logic documentation](</docs/edge-logic/intro.md>). You can click the [**Wizard** button](#edge-logic-wizard) to bootstrap this configuration using the Cache Settings dialog box. The following directives are available in this field. <li>`add_header` = modify the response header to the client.</li><li>`allow` = specify an allowed client IP address range.</li><li>`client_body_timeout` = timeout to wait for request body.</li><li>`client_header_timeout` = timeout to wait for request header.</li><li>`client_send_timeout` = timeout to send response to client.</li><li>`deny` = specify the denied client IP address range.</li><li>`expires` = set the cache time headers to the client.</li><li>`proxy_hide_header` = do not pass a header from the origin to the clients.</li><li>`proxy_pass_header` = pass a hidden header to the clients.</li><li>`return` = return code and message to the clients.</li> 
