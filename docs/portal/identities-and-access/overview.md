...
| 2 | To filter users on the **Users** tab or roles on the **Roles** tab, type characters in this field and then press the Enter key. All users or roles that do not contain the typed characters are hidden. Filtering is not case-sensitive. To remove the filter, click the **Reset** button.|
