...
This directive enables proxying the WebSocket protocol. The client has to make sure not to use HTTP/2. The read and send timeouts are set to 60s. It should not be used with `origin_read_timeout` or `origin_send_timeout` in the same context.
