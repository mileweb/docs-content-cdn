...
For more information about these two directives, please refer to the NGINX documentation site. On the portal, you need to enter only the code above into the "Edge Logic" text area:
...
Note: You must enter the entire Edge Logic code in one line following the [JSON string escaping](https://www.freeformatter.com/json-escape.html) syntax. Assume you configured the acceleration hostname to be `www.testdomain.com` as shown in the JSON above, and then deployed the property to the staging environment, you can use a cURL command to perform a test as shown below:
...
In this example, 163.171.228.89 is the IP address of one of the staging servers. You can find the details about the usage of the staging environment in [this section](</docs/edge-logic/staging-environment.md>). 
