...
*   Not all directives can be used in an `if` block. This is specified in the "Context" of each directive. For the directives not allowed in an `if` block, if they support a variable as a parameter, (for example [`proxy_cache_valid`](</docs/edge-logic/supported-directives.md#proxy_cache_valid>), [`proxy_redirect`](</docs/edge-logic/supported-directives.md#proxy_redirect>), [`proxy_cookie_domain`](</docs/edge-logic/supported-directives.md#proxy_cookie_domain>)) you can still control their behavior based on `if` conditions using the [`set`](</docs/edge-logic/supported-directives.md#set>)) directive.
