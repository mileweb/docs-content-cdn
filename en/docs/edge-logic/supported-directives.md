...
| cypher | ENCRYPT_SYMM<br>**DECRYPT_SYMM** | ```eval_func $output ENCRYPT_SYMM $key $iv $message $mode;```<br>```$key``` and ```$iv``` should both be binary strings. $mode can be any of the ciphers returned by <i>openssl list -cipher-commands</i>, for example, 'aes-128-cbc'. |
