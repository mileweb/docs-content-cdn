...
```
When both `header-a` and `header-b` are present in the request, only `has-header-b` will be added to the response. This is not a bug, but rather how Nginx works. However, this behavior can be counterintuitive to new users. Refer to [this page](declarative-imperative) for more technical details.
*   Not all directives can be used in an `if` block. This is documented in the "Context" of each directive. If a directive is not allowed in `if` blocks but supports variable as parameter, you can still control its behavior based on `if` conditions using the [`set`](</docs/edge-logic/supported-directives.md#set>) directive. For example: [`proxy_cache_valid`](</docs/edge-logic/supported-directives.md#proxy_cache_valid>), [`proxy_redirect`](</docs/edge-logic/supported-directives.md#proxy_redirect>) and [`proxy_cookie_domain`](</docs/edge-logic/supported-directives.md#proxy_cookie_domain>).
