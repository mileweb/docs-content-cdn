...
| Need this hostname to be resolved to edge servers in mainland China | Select whether traffic should be directed to regions in mainland China. Choices are:<br><ul><li><strong>No</strong> = traffic will not be directed to servers in mainland China. (*default*)</li><li><strong>Yes</strong> = traffic will be directed to servers in all regions including mainland China. Use this setting only for hostnames that have ICP Beian. Otherwise, requests directed to servers in mainland China will be rejected with the status code 451. </br></ul></li><strong> Note:</strong> This setting cannot be changed. If you need to change it, create a new edge hostname with the required setting and change DNS to use that edge hostname instead. </li> |
